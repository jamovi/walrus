...
ranovaClass<-if (requireNamespace('jmvcore')) R6::R6Class(
    ...
    private=list(
        ...
        .medpb=function(x, alpha=.05, nboot=NA, grp=NA, est=median, con=0, bhop=FALSE, method='hoch', SEED=TRUE,...){
        ...
        .pbadepth=function(x,est=onestep,con=0,alpha=.05,nboot=2000,grp=NA,op=3,allp=TRUE,MM=FALSE,MC=FALSE,cop=3,SEED=TRUE,na.rm=FALSE,...){
            ...
            pdis<-function(m,pts=m,MM=FALSE,cop=3,dop=1,center=NA,na.rm=TRUE){
                ...
                if(ncol(m)==1){
                    ...
                    if(!MM)pdis<-dis/(temp$qu-temp$ql)
